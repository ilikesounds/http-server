...
#    ('HTTP/1.1',
#     response_deconstructor(response_ok(TEXT, ('text/plain', None)))[0]),
#    ('200', response_deconstructor(response_ok(TEXT, ('text/plain', None)))[1]),
#    ('OK', response_deconstructor(response_ok(TEXT, ('text/plain', None)))[2]),
#    ('8', response_deconstructor(response_ok(TEXT, ('text/plain', None)))[4]),
#    ('15138', response_deconstructor(response_ok(JPEG[0], JPEG[1]))[4])
# ]
...
JPEG = resolve_uri('./webroot/images/JPEG_example.jpg')
TEXT = b"Success!"
...
def test_generate_response_text():
    """Test whether generate_response(uri) generates:
     a. - 404 error if file not found or
     b. - ok-message with body and mime if file found."""
    response = generate_response(RESOLVE_URI[0][0])
    decon = response_deconstructor(response)
    assert decon[3]['content-length'] == '95'
    #import pdb;pdb.set_trace()
    assert decon[3]['content-type'] == str(('text/plain', None))

def test_generate_response_jpeg():
    """Test whether generate_response(uri) generates:
     a. - 404 error if file not found or
     b. - ok-message with body and mime if file found."""
    response = generate_response(RESOLVE_URI[4][0])
    decon = response_deconstructor(response)
    assert decon[3]['content-type'] == str(('image/jpeg', None))
    assert decon[3]['content-length'] == '15138'
